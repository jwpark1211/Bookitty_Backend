...
public class BookService {
    ...
    public List<AladinBookSearchListResponse> getRecommendationsForUser(Long memberId) {
        ...
        log.info("5. 최종 스코어가 높은 순으로 정렬 후 상위 10개 뽑기");
        List<String> resultIsbns = adjustedRecommendationMap.entrySet().stream()
                .sorted(Map.Entry.<String, Double>comparingByValue().reversed())
                .limit(10)
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());

        log.info("6. 상위 10개의 isbn을 토대로 결과 dto 도출하기");
        return resultIsbns.stream()
